 ...
 * ----------------------------------------------------------------------------
 * This class is forked from the Apache Commons Math library, for the purpose
 * of adding needed functionality specific to this application.
 */
...
public class Fraction extends Number implements Comparable<Fraction>, Serializable {
    ...
    private static final long serialVersionUID = 4072354680160055065L;
    ...
    private Fraction(int num, int den, boolean reduce) {
        int requestedDen = FastMath.abs(den);
        if (den == 0) {
        ...
        if (!reduce && requestedDen > den) {
            int multiple = requestedDen / den;
            num = num * multiple;
            den = requestedDen;
        }
    ...
    /**
     * <p>Creates a {@code Fraction} instance with the 2 parts
     * of a fraction Y/Z, without reducing that fraction.</p>
     *
     * <p>Any negative signs are resolved to be on the numerator.</p>
     *
     * @param numerator  the numerator, for example the three in 'three sevenths'
     * @param denominator  the denominator, for example the seven in 'three sevenths'
     * @return a new fraction instance, with the numerator and denominator unreduced
     * @throws MathArithmeticException if the denominator is {@code zero}
     */
    public static Fraction getUnreducedFraction(int numerator, int denominator) {
